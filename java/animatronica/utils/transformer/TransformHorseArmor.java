...
public class TransformHorseArmor implements IClassTransformer, Opcodes{
	...
	private byte[] transformHorse(byte[] bytes){
		...
		transformInteract(ASMHelper.findMethod(ASMNames.M_interact, "(Lnet/minecraft/entity/player/EntityPlayer;)Z", clazz));
		transformIsValidArmor(ASMHelper.findMethod(ASMNames.M_func146085a, "(Lnet/minecraft/item/Item;)Z", clazz));
		transformEntityInit(ASMHelper.findMethod(ASMNames.M_entityInit, "()V", clazz));
		transformUpdateHorseSlots(ASMHelper.findMethod(ASMNames.M_func110232cE, "()V", clazz));
		transformOnInvChanged(ASMHelper.findMethod(ASMNames.M_onInventoryChanged, "(Lnet/minecraft/inventory/InventoryBasic;)V", clazz));
		transformGetTotalArmorValue(ASMHelper.findMethod(ASMNames.M_getTotalArmorValue, "()I", clazz));

		try{
			transformArmorTexture(ASMHelper.findMethod(ASMNames.M_setHorseTexturePaths, "()V", clazz));
		}catch(RuntimeException e){
			Animatronica.logger.info("Running on dedicated server, no need to transform Method %s!", ASMNames.M_setHorseTexturePaths);
		}
	...
	private void transformGetTotalArmorValue(MethodNode method){
		...
		newInstr.add(new TypeInsnNode(INSTANCEOF, "agravaine.agravainelib.item.armor.ItemHorseArmorBase"));
		LabelNode l2 = new LabelNode();
		newInstr.add(new JumpInsnNode(IFEQ, l2));
		newInstr.add(new LabelNode());
		newInstr.add(new VarInsnNode(ALOAD, 1));
		newInstr.add(new MethodInsnNode(INVOKEVIRTUAL, "net/minecraft/item/ItemStack", ASMNames.M_getItem, "()Lnet/minecraft/item/Item;"));
		newInstr.add(new TypeInsnNode(CHECKCAST, "agravaine.agravainelib.item.armor.ItemHorseArmorBase"));
		newInstr.add(new VarInsnNode(ALOAD, 0));
		newInstr.add(new VarInsnNode(ALOAD, 1));
		newInstr.add(new MethodInsnNode(INVOKEVIRTUAL, "agravaine.agravainelib.item.armor.ItemHorseArmorBase", "getArmorValue", "(Lnet/minecraft/entity/passive/EntityHorse;Lnet/minecraft/item/ItemStack;)I"));
	...
	private void transformArmorTexture(MethodNode method){
		...
		newInstr.add(new VarInsnNode(ALOAD, 0));
		newInstr.add(new VarInsnNode(ALOAD, 4));
		newInstr.add(new MethodInsnNode(INVOKEVIRTUAL, "agravaine.agravainelib.item.armor.ItemHorseArmorBase", "getArmorTexture", "(Lnet/minecraft/entity/passive/EntityHorse;Lnet/minecraft/item/ItemStack;)Ljava/lang/String;"));
