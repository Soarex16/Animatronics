...
public class Utils{
	...
	public static void restartMinecraftApplet(){
		...
		try{
			Animatronica.logger.info("---=== Restarting Minecraft Client! ===---");    // try to shutdown Minecraft applet
			Minecraft.getMinecraft().shutdownMinecraftApplet();
		}catch(NoClassDefFoundError e){
			Animatronica.logger.info("---=== Restarting Minecraft Server! ===---");    // if Minecraft class was not found                                                                    // (usually the case on a dedi-server),
	...
	public static void openInBrowser(String url){
		if(Desktop.isDesktopSupported()){
			try{
				...
				Animatronica.logger.error("Failed to browse " + url + "!");
			}
		}
	}
}
