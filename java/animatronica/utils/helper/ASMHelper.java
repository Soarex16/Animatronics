...
public class ASMHelper{
	...
	public static byte[] createBytes(ClassNode cnode, int flags){
		...
		Animatronica.logger.info("Class %s successfully transformes!", cnode.name);
		return byteArray;
	}
	...
	public static AbstractInsnNode findLastNodeFromNeedle(InsnList haystack, InsnList needle){
		List<AbstractInsnNode> returnList = insnListFindEnd(haystack, needle);
		if(returnList.size() != 1){
			throw new RuntimeException(returnList.size() > 1 ? "Multiple Needles found in Haystack!" : "Needle not found in Haystack!");
		}
		return returnList.get(0);
	}

	public static AbstractInsnNode findFirstNodeFromNeedle(InsnList haystack, InsnList needle){
		List<AbstractInsnNode> returnList = insnListFindStart(haystack, needle);
		if(returnList.size() != 1){
			throw new RuntimeException(returnList.size() > 1 ? "Multiple Needles found in Haystack!" : "Needle not found in Haystack!");
		}
		return returnList.get(0);
	...
	public static MethodNode findMethod(String name, String desc, ClassNode cnode){
		...
		throw new RuntimeException(String.format("Could not find any method matching the name < %s > and description < %s >", name, desc));
	}
	...
	public static InsnListSection insnListMatchesL(InsnList haystack, InsnList needle, int start, HashSet<LabelNode> controlFlowLabels){
		int h = start;
		int n = 0;
